...
class wms_report_stock_available(osv.osv):
    ...
    def get_qty(self, cr, uid, product_id, location_id, context=None):
        search_ids = self.search(cr, uid, [('product_id', '=', product_id), ('location_id', '=', location_id)],
                                 context=context)
...
FROM (
   SELECT   -max(sm.id) AS id,
        sm.location_id,
        sm.product_id,
        sm.restrict_lot_id as prodlot_id,
        sl.usage,
   ...
   GROUP BY sm.location_id, sm.product_id, sm.product_uom, sm.restrict_lot_id, sl.usage
   UNION ALL
   SELECT   max(sm.id) AS id,
   ...
   LEFT JOIN product_uom uo ON (uo.id=sm.product_uom)
   WHERE   sm.location_id != sm.location_dest_id
   GROUP BY sm.location_dest_id, sm.product_id, sm.product_uom, sm.restrict_lot_id, sl.usage
