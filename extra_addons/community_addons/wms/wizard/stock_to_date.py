...
class stock_to_date(osv.osv_memory):
        ...
        for wizard in self.browse(cr, uid, ids, context=context):
            ...
            if not warehouse_ids:
                raise osv.except_osv(_('Warning !'),
                                     _('Please contact your administrator to configure warehouse in your profile.'))
    ...
    def _get_orderpoint(self, cr, uid, ids, field_name, args, context=None):
        ...
        result = {}
        for wizard in self.browse(cr, uid, ids, context=context):
            result[wizard.id] = orderpoint_obj.search(cr, uid, [('product_id', '=', wizard.product_id.id)],
                                                      context=context)
        return result
    ...
    def _get_report_stock(self, cr, uid, ids, field_name, args, context=None):
        ...
        result = {}
        for wizard in self.browse(cr, uid, ids, context=context):
            result[wizard.id] = report_obj.search(cr, uid, [('usage', '=', 'internal'),
                                                            ('product_id', '=', wizard.product_id.id)], context=context)
        return result
    ...
    _columns = {
        ...
        'stock_to_date_line_ids': fields.one2many('stock.to.date.line', 'stock_to_date_id', 'Line of stock to date',
                                                  readonly=True),
        'warehouse_id': fields.many2one('stock.warehouse', 'Warehouse', required=False),
        'orderpoint_ids': fields.function(_get_orderpoint, method=True, string='OrderPoint', type='one2many',
                                          relation='stock.warehouse.orderpoint', store=False),
        'report_stock_ids': fields.function(_get_report_stock, method=True, string='Stock Available', type='one2many',
                                            relation='wms.report.stock.available', store=False),
    }
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        product_id = 'default_product_id' in context and context['default_product_id'] or 'active_id' in context and \
                                                                                          context['active_id'] or False
        orderpoint_obj = self.pool.get('stock.warehouse.orderpoint')
        report_obj = self.pool.get('wms.report.stock.available')
        ...
        if product_id:
            ...
            report_stock_ids = report_obj.search(cr, uid, [('usage', '=', 'internal'), ('product_id', '=', product_id)],
                                                 context=context)
