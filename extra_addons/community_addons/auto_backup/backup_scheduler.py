...
try:
    ...
    _logger.error("pysftp not installed")
        ...
        for rec in confs:
            ...
            if rec.name in db_list:
                ...
                try:
                    ...
                    _logger.info("Couldn't backup database %s. Bad database administrator password for server running at http://%s:%s" %(rec.name, rec.host, rec.port))
                    continue
