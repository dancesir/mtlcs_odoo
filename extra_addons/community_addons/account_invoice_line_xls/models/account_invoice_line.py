...
class AccountInvoiceLine(models.Model):
    ...
    invoice_number = fields.Char(
        related='invoice_id.number', string=u'发票', readonly=True)
    invoice_type = fields.Selection(
        related='invoice_id.type', string=u'类型', readonly=True)
    invoice_state = fields.Selection(
        related='invoice_id.state', string=u'状态', readonly=True)
    invoice_date = fields.Date(
        related='invoice_id.date_invoice', string=u'开票日期',
        readonly=True)
    invoice_journal_id = fields.Many2one(
        related='invoice_id.journal_id', string=u'账簿', readonly=True)
    invoice_partner_id = fields.Many2one(
        related='invoice_id.partner_id', string=u'供应商', readonly=True)
