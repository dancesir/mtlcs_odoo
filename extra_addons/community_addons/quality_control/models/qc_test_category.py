...
class QcTestTemplateCategory(models.Model):
    ...
    name = fields.Char(u'分类', required=True, translate=True)
    parent_id = fields.Many2one(
        comodel_name='qc.test.category', string=u'上级分类', select=True)
    complete_name = fields.Char(
        compute="_get_complete_name", string=u'完整名')
    child_ids = fields.One2many(
        comodel_name='qc.test.category', inverse_name='parent_id',
        string=u'子分类')
    active = fields.Boolean(
        string=u'有效', default=True,
